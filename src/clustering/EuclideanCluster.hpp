...
class EuclideanCluster {
 public:
  ...
  std::vector<typename pcl::PointCloud<PointT>::Ptr> estimate(
    ...
    for (int i = 0; i < cloud->size(); ++i) {
      ...
      if (!(cluster->size() < m_minSize)) clusters.push_back(cluster);
      i++;
    }
  ...
  void clusterRecursive(int i, typename pcl::PointCloud<PointT>::Ptr cloud,
                        typename pcl::PointCloud<PointT>::Ptr cluster,
                        std::vector<bool>& processed) {
    processed[i] = true;
    if (cluster->size() > m_maxSize) return;
    cluster->push_back(cloud->points[i]);
    std::vector<int> nearest = m_tree->search(cloud->points[i], m_distanceTol);
    for (int idx : nearest) {
      if (!processed[idx]) clusterRecursive(idx, cloud, cluster, processed);
    }
  }
